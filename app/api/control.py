...
# Every control endpoint requires a logged-in officer (Depends(require_auth)).
...
@router.post("/connect")
def connect(user=Depends(require_auth)):
    res = get_adapter().connect()
    return {"ok": res.ok, "detail": res.detail, "data": res.data}


@router.post("/give-title")
def give_title(body: GiveTitleIn, user=Depends(require_auth)):
...
def change_rank(body: ChangeRankIn, user=Depends(require_auth)):
    return _enqueue("change_rank", user, player_id=body.player_id,
                    params={"new_rank": body.new_rank})


@router.post("/locate")
def locate(body: LocateIn, user=Depends(require_auth)):
    return _enqueue("locate", user, player_id=body.player_id)


@router.post("/scan")
def scan(body: ScanJobIn, user=Depends(require_auth)):
    return _enqueue("scan", user, params={"kind": body.kind, "pages": body.pages})


@router.get("/commands")
def commands(limit: int = 50, user=Depends(require_auth)):
